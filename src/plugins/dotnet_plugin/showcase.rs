...
mod tests {
    ...
    /// 从 `samples/dotnet_plugin` 目录读取示例日志文本，文件缺失时返回空串。
    ...
    fn compress_text(plugin: &DotNetPlugin, text: &str) -> String {
        let slice = Slice {
            id: 1,
    ...
    /// 生成 .NET 压缩全景 showcase 报告：遍历多组样例，输出原文/压缩行数字节与压缩率到文件。
    #[test]
    fn generate_dotnet_showcase_report() {
        let plugin = DotNetPlugin::new();
        let cases = [
            ("case_001_build_success", "Build 成功"),
            ("case_002_build_error", "Build 错误"),
            ("case_003_stack_trace", "堆栈跟踪"),
            ("case_004_msbuild", "MSBuild 输出"),
            ("case_005_test_results", "测试结果"),
            ("case_006_noise", "噪音夹杂"),
            ("case_007_long_line", "超长行"),
            ("case_008_single_line", "单行输入"),
            ("case_009_empty", "空输入"),
            ("case_010_mixed", "混合场景"),
            ("case_011_special_chars", "特殊字符"),
            ("case_012_no_compress", "不压缩场景"),
        ];

        let mut all_output = String::new();
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n  .NET AI Compact Showcase\n");
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n\n");
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("dotnet_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
