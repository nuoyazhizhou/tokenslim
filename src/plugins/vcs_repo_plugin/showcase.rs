...
mod tests {
    ...
    /// 测试：遍历 repo 样例生成 showcase 对比报告并写入 target 目录。
    #[test]
    fn generate_showcase_report() {
        ...
        let cases: &[&str] = &[
            ...
            "case_117_repo_branches",
            "case_118_repo_diff",
            "case_119_repo_start",
            "case_120_repo_checkout",
            "case_121_repo_forall",
            "case_122_repo_stage",
            "case_123_repo_init",
            "case_124_repo_upload",
        ];
        let mut out = String::new();
        out.push_str(&"=".repeat(80));
        out.push_str("\n  VCS Repo AI Compact Showcase - Detailed Case-by-Case Report\n");
        out.push_str(&"=".repeat(80));
        out.push_str("\n\n");
        ...
        let op = std::path::Path::new(manifest_dir)
            .join("target")
            .join("vcs_repo_compact_showcase_report.txt");
        std::fs::write(&op, &out).unwrap();
        eprintln!("\n[Repo Showcase] {}", op.display());
    }
}
