...
mod tests {
    ...
    fn generate_xml_html_showcase_report() {
        ...
        let cases = [
            ("case_001_simple_xml.log", "简单XML"),
            ("case_002_simple_html.log", "简单HTML"),
            ("case_003_complex_xml.log", "复杂XML"),
            ("case_004_svg.log", "SVG"),
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("xml_html_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
