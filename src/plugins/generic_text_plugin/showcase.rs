...
mod tests {
    ...
    fn generate_generic_text_showcase_report() {
        ...
        let cases = [
            ("case_001_normal_text.log", "普通文本"),
            ("case_002_ansi_colors.log", "ANSI 颜色代码"),
            ("case_003_many_blank_lines.log", "大量空行"),
            ("case_004_tabs_and_spaces.log", "制表符和空格"),
            ("case_005_trailing_whitespace.log", "尾部空白"),
            ("case_006_carriage_return.log", "回车覆盖行"),
            ("case_007_long_line.log", "长文本行"),
            ("case_008_single_line.log", "单行文本"),
            ("case_009_empty.log", "空输入"),
        ...
        all_output.push_str("\n  Generic Text AI Compact Showcase\n");
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n\n");
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("generic_text_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
