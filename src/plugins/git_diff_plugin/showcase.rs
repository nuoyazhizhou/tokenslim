...
mod tests {
    ...
    /// 从 `samples/git_diff_plugin` 目录读取示例日志文本，文件缺失时返回空串。
    fn read_sample(file_name: &str) -> String {
        let manifest_dir = env!("CARGO_MANIFEST_DIR");
        let path = std::path::Path::new(manifest_dir)
            .join("samples")
            .join("git_diff_plugin")
            .join(file_name);
        std::fs::read_to_string(&path).unwrap_or_default()
    }

    /// 用插件对文本切片做压缩，拼接所有 `Text` token 组成紧凑字符串返回。
    fn compress_text(plugin: &GitDiffPlugin, text: &str) -> String {
        let slice = Slice {
            id: 1,
    ...
    /// 生成 Git Diff 压缩全景 showcase 报告：遍历多组样例，输出原文/压缩行数字节与压缩率到文件。
    #[test]
    fn generate_git_diff_showcase_report() {
        let plugin = GitDiffPlugin::new();
        let cases = [
            ("case_001_simple_change.log", "简单变更"),
            ("case_002_multiple_changes.log", "多个文件变更"),
            ("case_003_large_diff.log", "大型差异"),
            ("case_004_deletion.log", "文件删除"),
            ("case_005_rename.log", "文件重命名"),
            ("case_006_binary_file.log", "二进制文件"),
            ("case_007_merge_conflict.log", "合并冲突"),
            ("case_008_empty_diff.log", "空差异"),
            ("case_009_single_line_change.log", "单行变更"),
            ("case_010_mode_change.log", "模式变更"),
            ("case_011_no_compress.log", "不压缩场景"),
            ("case_012_multiple_hunks.log", "多个代码块"),
        ];

        let mut all_output = String::new();
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n  Git Diff AI Compact Showcase\n");
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n\n");
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("git_diff_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
