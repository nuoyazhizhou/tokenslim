...
mod tests {
    ...
    fn generate_xcode_log_showcase_report() {
        ...
        let cases = [
            ("case_001_compile", "CompileC 编译"),
            ("case_002_probe", "Clang 探针"),
            ("case_003_linking", "Linking 链接"),
            ("case_004_cd", "cd 命令"),
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("xcode_log_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
