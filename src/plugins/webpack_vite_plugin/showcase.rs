...
mod tests {
    ...
    /// 测试辅助：读取 samples/webpack_vite_plugin 目录下的样例文件。
    fn read_sample(file_name: &str) -> String {
        let manifest_dir = env!("CARGO_MANIFEST_DIR");
        let path = std::path::Path::new(manifest_dir)
            .join("samples")
            .join("webpack_vite_plugin")
            .join(file_name);
        std::fs::read_to_string(&path).unwrap_or_default()
    }

    /// 测试辅助：构造 Slice 并调用插件 compress，拼接 Text token 得到压缩文本。
    fn compress_text(plugin: &WebpackVitePlugin, text: &str) -> String {
        let slice = Slice {
            id: 1,
    ...
    /// 测试：遍历样例生成 webpack_vite 插件的 showcase 对比报告并写入 target 目录。
    #[test]
    fn generate_webpack_vite_showcase_report() {
        let plugin = WebpackVitePlugin::new();
        let cases = [
            ("case_001_build_success.log", "构建成功"),
            ("case_002_build_error.log", "构建错误"),
            ("case_003_asset_table.log", "资源表"),
            ("case_004_dev_server.log", "开发服务器"),
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("webpack_vite_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
