...
mod tests {
    ...
    /// 测试辅助：读取 samples/kubernetes_docker_plugin 目录下的样例文件。
    ...
    fn compress_text(plugin: &KubernetesDockerPlugin, text: &str) -> (String, String) {
        let slice = Slice {
            id: 1,
    ...
    /// 测试：遍历样例生成 kubernetes_docker 插件的 showcase 对比报告并写入 target 目录。
    #[test]
