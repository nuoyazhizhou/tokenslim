...
impl VcsParser for SvnLogParser {
    ...
    fn parse(&self, raw: &str) -> Option<VcsDocument> {
        let mut records = Vec::new();
        let mut skip_changed_paths = false;
        ...
        for line in raw.lines() {
            ...
            if trimmed.eq_ignore_ascii_case("changed paths:") {
                skip_changed_paths = true;
                continue;
            }
            if skip_changed_paths {
                continue;
            }

            if let Some(rest) = trimmed.strip_prefix('r') {
                // 拍扁：将上一个 revision 的 message 行合并到 header
                if last_was_header && !pending_message.is_empty() {
                    let header = records.last_mut().unwrap();
                    if let VcsRecord::Raw(h) = header {
                        h.push_str(" | CM:");
