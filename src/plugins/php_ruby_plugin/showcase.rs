...
mod tests {
    ...
    /// 测试辅助：读取 samples/php_ruby_plugin 目录下的样例文件。
    fn read_sample(file_name: &str) -> String {
        let manifest_dir = env!("CARGO_MANIFEST_DIR");
        let path = std::path::Path::new(manifest_dir)
            .join("samples")
            .join("php_ruby_plugin")
            .join(file_name);
        std::fs::read_to_string(&path).unwrap_or_default()
    }

    /// 测试辅助：构造 Slice 并调用插件 compress，拼接 Text token 得到压缩文本。
    fn compress_text(plugin: &PhpRubyPlugin, text: &str) -> String {
        let slice = Slice {
            id: 1,
    ...
    /// 测试：遍历样例生成 php_ruby 插件的 showcase 对比报告并写入 target 目录。
    #[test]
    fn generate_php_ruby_showcase_report() {
        let plugin = PhpRubyPlugin::new();
        let cases = [
            ("case_001_php_error.log", "PHP错误"),
            ("case_002_ruby_error.log", "Ruby错误"),
            ("case_003_mixed.log", "混合场景"),
            ("case_004_long_stack.log", "长堆栈"),
            ("case_005_single_line.log", "单行日志"),
            ("case_006_empty.log", "空日志"),
            ("case_007_special_chars.log", "特殊字符"),
            ("case_008_noise.log", "带噪声"),
            ("case_009_mixed_content.log", "混合内容"),
            ("case_010_no_compress.log", "不压缩"),
            ("case_011_rails.log", "Rails日志"),
            ("case_012_laravel.log", "Laravel日志"),
        ];

        let mut all_output = String::new();
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n  PHP/Ruby AI Compact Showcase\n");
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n\n");
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("php_ruby_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
