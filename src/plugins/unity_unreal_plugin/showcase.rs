...
mod tests {
    ...
    fn generate_unity_unreal_showcase_report() {
        ...
        let cases = [
            ("case_001_unity_log.log", "Unity日志"),
            ("case_002_unreal_log.log", "Unreal日志"),
            ("case_003_build.log", "构建日志"),
            ...
            ("case_010_performance.log", "性能"),
            ("case_011_collision.log", "碰撞"),
            ("case_012_complex.log", "复杂场景"),
            ("case_013_real_unreal_logs.log", "真实Unreal日志"),
            ("case_014_real_unity_logs.log", "真实Unity日志"),
            ("case_015_looks_unreal_but_syslog.log", "伪Unreal-Syslog"),
        ];

        let mut all_output = String::new();
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n  Unity Unreal AI Compact Showcase\n");
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n\n");
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("unity_unreal_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
