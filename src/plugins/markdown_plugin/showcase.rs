...
mod tests {
    ...
    /// 从 `samples/markdown_plugin` 目录读取示例文本，文件缺失时返回空串。
    fn read_sample(file_name: &str) -> String {
        let manifest_dir = env!("CARGO_MANIFEST_DIR");
        let path = std::path::Path::new(manifest_dir)
            .join("samples")
            .join("markdown_plugin")
            .join(file_name);
        std::fs::read_to_string(&path).unwrap_or_default()
    }

    /// 用插件对文本切片做压缩，拼接所有 `Text` token 组成紧凑字符串返回。
    fn compress_text(plugin: &MarkdownPlugin, text: &str) -> String {
        let slice = Slice {
            id: 1,
    ...
    /// 从样例文件名提取用例 ID（取首个 `.` 之前的部分）。
    fn extract_case_id(file_name: &str) -> &str {
        file_name.split('.').next().unwrap_or(file_name)
    }

    /// 生成 Markdown 压缩全景 showcase 报告：遍历多组样例，输出原文/压缩行数字节与压缩率到文件。
    #[test]
    fn generate_markdown_showcase_report() {
        let plugin = MarkdownPlugin::new();
        let cases = [
            ("case_001_simple.md", "简单Markdown"),
            ("case_002_code.md", "代码块"),
            ("case_003_table.md", "表格"),
            ("case_004_links.md", "链接"),
            ("case_005_headers.md", "标题"),
            ("case_006_noise.log", "带噪声"),
            ("case_007_single_line.md", "单行Markdown"),
            ("case_008_empty.md", "空输入"),
            ("case_009_special_chars.md", "特殊字符"),
            ("case_010_mixed.log", "混合场景"),
            ("case_011_no_compress.txt", "不压缩场景"),
            ("case_012_complex.md", "复杂Markdown"),
        ];

        let mut all_output = String::new();
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n  Markdown AI Compact Showcase\n");
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n\n");
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("markdown_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
