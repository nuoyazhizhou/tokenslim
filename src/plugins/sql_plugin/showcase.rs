...
mod tests {
    ...
    fn generate_sql_showcase_report() {
        ...
        let cases = [
            ("case_001_select.log", "SELECT语句"),
            ("case_002_error.log", "错误"),
            ("case_003_complex.log", "复杂查询"),
            ("case_004_insert.log", "INSERT语句"),
            ...
            ("case_011_transaction.log", "事务"),
            ("case_012_complex_join.log", "复杂JOIN"),
        ];

        let mut all_output = String::new();
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n  SQL AI Compact Showcase\n");
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n\n");
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("sql_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
