...
mod tests {
    ...
    /// 测试辅助：读取 samples/static_rule_plugin 目录下的样例文件。
    fn read_sample(file_name: &str) -> String {
        let manifest_dir = env!("CARGO_MANIFEST_DIR");
        let path = std::path::Path::new(manifest_dir)
            .join("samples")
            .join("static_rule_plugin")
            .join(file_name);
        std::fs::read_to_string(&path).unwrap_or_default()
    }

    /// 测试辅助：构造 Slice 并调用插件 compress，拼接 Text token 得到压缩文本。
    fn compress_text(plugin: &SimpleRulePlugin, text: &str) -> String {
        let slice = Slice {
            id: 1,
    ...
    /// 测试：遍历样例生成 static_rule 插件的 showcase 对比报告并写入 target 目录。
    #[test]
    fn generate_static_rule_showcase_report() {
        let plugin = SimpleRulePlugin::new(StaticRuleConfig::default());
        let cases = [
            ("case_001_rule_check.log", "规则检查"),
            ("case_002_empty.log", "空内容"),
            ("case_003_single_line.log", "单行"),
            ("case_004_no_compress.log", "不压缩"),
            ("case_005_mixed.log", "混合内容"),
            ("case_006_special_chars.log", "特殊字符"),
            ("case_007_noise.log", "带噪声"),
            ("case_008_complex.log", "复杂场景"),
            ("case_009_pass.log", "通过场景"),
            ("case_010_warnings.log", "警告"),
            ("case_011_errors.log", "错误"),
            ("case_012_performance.log", "性能"),
        ];

        let mut all_output = String::new();
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n  Static Rule AI Compact Showcase\n");
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n\n");
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("static_rule_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
