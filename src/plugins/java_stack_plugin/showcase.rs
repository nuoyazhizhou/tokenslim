...
mod tests {
    ...
    /// 测试辅助：读取 samples/java_stack_plugin 目录下的样例文件。
    fn read_sample(file_name: &str) -> String {
        let manifest_dir = env!("CARGO_MANIFEST_DIR");
        let path = std::path::Path::new(manifest_dir)
            .join("samples")
            .join("java_stack_plugin")
            .join(file_name);
        std::fs::read_to_string(&path).unwrap_or_default()
    }

    /// 测试辅助：构造 Slice 并调用插件 compress，拼接 Text token 得到压缩文本。
    /// 返回 (压缩文本, 字典侧通道 JSON)。字典 JSON 由字典引擎快照生成，携带可逆 token->原文 映射。
    fn compress_text(plugin: &JavaStackPlugin, text: &str) -> (String, String) {
        let slice = Slice {
            id: 1,
        ...
        let compacted = result
            ...
            .collect::<String>();
        let dict_json = crate::plugins::test_utils::full_dict_json(&dict, &compacted);
        (compacted, dict_json)
    }

    /// 测试：遍历样例生成 java_stack 插件的 showcase 对比报告并写入 target 目录。
    #[test]
    fn generate_java_stack_showcase_report() {
        let plugin = JavaStackPlugin::new();
        let cases = [
            ("case_001_simple_exception.log", "简单异常"),
            ("case_002_nested_exception.log", "嵌套异常"),
            ("case_003_long_stack.log", "长堆栈"),
            ("case_004_runtime_exception.log", "运行时异常"),
            ("case_005_chained_exceptions.log", "链式异常"),
            ("case_006_noise.log", "带噪声日志"),
            ("case_007_single_line.log", "单行异常"),
            ("case_008_empty.log", "空输入"),
            ("case_009_special_chars.log", "特殊字符"),
            ("case_010_mixed.log", "混合场景"),
            ("case_011_no_compress.log", "不压缩场景"),
            ("case_012_lambda_exception.log", "Lambda异常"),
            ("case_013_duplicate_stack.log", "重复堆栈"),
            ("case_014_deep_stack.log", "深层堆栈"),
            ("case_015_exception_summary.log", "异常摘要"),
            ("case_016_suppressed.log", "Suppressed 异常"),
        ];

        let mut all_output = String::new();
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n  Java Stack AI Compact Showcase\n");
        all_output.push_str(&"=".repeat(80));
        all_output.push_str("\n\n");
        ...
        for (file_name, title) in cases {
            ...
            // 字典侧通道段（dictside）：跟随 compact 段之后，携带可逆 token->原文 映射。
            // 审计脚本在遇到 "-- Dictionary (full) --" 时停止读取 compact，故不会改变 compact 哈希。
        ...
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("target")
                .join("java_stack_compact_showcase_report.txt"),
            &all_output,
        )
        .unwrap();
    }
